...
function elCustomPrompt(containerName, title, placeholder) {
...
//显示登录框
function showLoginPrompt(containerName) {
...
function windowShowModalDialog(cName)
{
    var resCount = "";
    var i = 10;
    var isLogin = false;
...
function getMessagePlugin(obj, callback) {
    var password;
    var isLogin = false;
 ...
 * @return  0               成功
            非0             失败
 */
function SOF_SetEncryptMethod(EncryptMethod) {
    if(browserType === BROWSER_TYPE_NPAPI) {
        if(plugin() === null || typeof plugin().valid === "undefined") {
            return SOR_TimeoutErr;
        }
 ...
 * @return  非空            Base64编码的签名证书字符串
            空              失败
 */
function SOF_ExportUserCert(ContainerName) {
    if(browserType === BROWSER_TYPE_NPAPI) {
        if(plugin() === null || typeof plugin().valid === "undefined") {
            return nullStr;
        }
 ...
 * @return  非空                    证书内指定类型的信息
            空                      失败
 */
function SOF_GetCertInfo(Base64EncodeCert, type) {
    if(browserType === BROWSER_TYPE_NPAPI) {
        if(plugin() === null || typeof plugin().valid === "undefined") {
            return nullStr;
        }
 ...
 * @return  非空            Base64编码的PKCS#1格式签名值
            空              失败
 */
function SOF_SignData(ContainerName, InData) {
    if(browserType === BROWSER_TYPE_NPAPI) {
        if(plugin() === null || typeof plugin().valid === "undefined") {
            return nullStr;
        }
 ...
 * @return  非空            Base64编码的PKCS#7格式签名值
            空              失败
 */
function SOF_SignMessage(flag, ContainerName, InData) {
    if(browserType === BROWSER_TYPE_NPAPI) {
        if(plugin() === null || typeof plugin().valid === "undefined") {
            return nullStr;
        }
