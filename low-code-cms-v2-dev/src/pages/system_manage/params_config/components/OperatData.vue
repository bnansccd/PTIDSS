...
const submit = (refElForm: FormInstance | null) => {
    refElForm?.validate(async (valid, fields) => {
        if (valid) {
            const bool =
                props.id === "0"
                    ? await apiPostAdd({...ajaxParams, basic: '1'})
...
<template>
    <GlobalElDialog :="$attrs" @close="emit('close')">
        ...
        >
            ...
            <GlobalElFormItem v-if="!(props.id != 0)" label="code" prop="code">
                <el-input
                    :disabled="props.id != 0"
            ...
            <GlobalElFormItem v-if="!(props.id != 0)" label="系统内置" :prop="props.id != 0 ? '' : 'configType'">
                <el-input
                    :disabled="props.id != 0"
            ...
            <GlobalElFormItem label="参数键值" prop="configValue">
                <!-- <el-input
                    ...
                    placeholder="请输入参数键值"
                /> -->
                <el-radio-group v-model="ajaxParams.configValue">
