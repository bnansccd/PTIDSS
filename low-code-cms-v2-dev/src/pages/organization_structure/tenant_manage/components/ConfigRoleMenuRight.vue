...
import { apiPostMenu, apiGetAppAllMenuIds, apiGetAppMenuIds } from "./api";
...
onBeforeMount(async () => {
    loading.value = true;
    const data = await apiGetAppMenuIds(props.id, props.tenantId);
    ajaxParams.menuIds = data.map((item: any) => item.menuId);
    options.value = await apiGetAppAllMenuIds(props.id);
    loading.value = false;
});
...
async function submit(refElForm: FormInstance | null) {
    if (ajaxParams.checkIds.length === 0) {
    ...
    const bool = await apiPostMenu(ajaxParams, props.tenantId, props.id);
