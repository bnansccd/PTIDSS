...
<template>
    ...
    <WrapTableLayout @query="getList()" @reset="reset" more>
        <template #header>
            <WrapToolbarSearchLayout label="证书持有人">
                <GlobalSearchInput
                    @query="getList()"
                    v-model="ajaxParams.cn"
                    placeholder="请输入证书持有人查询"
                ></GlobalSearchInput>
            </WrapToolbarSearchLayout>
            <WrapToolbarSearchLayout label="证书编号">
                <GlobalSearchInput
                    @query="getList()"
                    v-model="ajaxParams.sn"
                    placeholder="请输入证书编号查询"
                ></GlobalSearchInput>
            </WrapToolbarSearchLayout>
        </template>
        ...
        >
            ...
            <vxe-column title="证书持有人" field="cn" >
                <template #default="{ row }">
                    {{ row.cn }}
                </template>
            </vxe-column>
            <vxe-column title="证件号码" field="sn" >
                <template #default="{ row }">
                    {{ row.sn }}
                </template>
            </vxe-column>
            <vxe-column title="系统账户">
                <template #default="{ row }">
                    {{ row.userVO?.username }}
                </template>
            </vxe-column>
