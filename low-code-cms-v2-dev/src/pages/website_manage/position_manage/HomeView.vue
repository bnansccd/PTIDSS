...
async function getList() {
    ...
    let { total, records } = await apiGetList(ajaxParams);
...
const del = (val: string | Array<string>) => {
    ...
    ElMessageBox.confirm("此操作将永久删除选择数据, 是否继续?", "提示", {
        ...
        .then(async () => {
            let bool = await apiDelete(Array.isArray(val) ? ids : val);
...
<template>
    ...
    <WrapTableLayout :showHeader="false" @query="getList()" @reset="reset" more>
        <template #header>
            <!-- <GlobalSearchInput
                @query="getList()"
        ...
        >
            ...
            <vxe-column title="岗位名称" field="postName">
                <template #default="{ row }">
                    {{ row.postName }}
                </template>
            </vxe-column>

            <vxe-column title="所属组织" field="departName">
                <template #default="{ row }">
                    {{ row.departName }}
                </template>
            </vxe-column>

            <vxe-column title="招聘人数" field="num">
                <template #default="{ row }">
                    {{ row.num }}
                </template>
            </vxe-column>

            <vxe-column field="publishDate" title="发布时间"> </vxe-column>
            <!-- <vxe-column field="jobDetail" title="内容"> </vxe-column> -->
