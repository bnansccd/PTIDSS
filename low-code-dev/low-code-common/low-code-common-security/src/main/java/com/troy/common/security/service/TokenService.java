package com.troy.common.security.service;
...
public class TokenService {
    ...
    private final static long REFRESH_TOKEN_EXPIRE = CacheConstants.REFRESH_TOKEN_EXPIRE;
     ...
     * 创建令牌
     */
    public LoginTokenVO createToken(LoginUser loginUser, Long appId) {
        String token = IdUtils.fastUUID();

        Long userId = loginUser.getSysUserVO().getId();
        String userName = loginUser.getSysUserVO().getUsername();
        loginUser.setToken(token);
        loginUser.setUserid(userId);
        loginUser.setUsername(userName);
        loginUser.setPhone(loginUser.getSysUserVO().getPhone());
        ...
        claimsMap.put(SecurityConstants.TENANT_ID, loginUser.getTenantId());
        claimsMap.put(SecurityConstants.DEPART_ID, loginUser.getSysDepartVO() == null ? null : loginUser.getSysDepartVO().getId());
        claimsMap.put(SecurityConstants.DEPART_NAME, loginUser.getSysDepartVO() == null ? null : loginUser.getSysDepartVO().getDepartName());

        // 接口返回信息
        LoginTokenVO loginTokenVO=new LoginTokenVO();
        loginTokenVO.setAccess_token(JwtUtils.createToken(claimsMap));
        loginTokenVO.setExpires_in(EXPIRE_TIME);
        return loginTokenVO;
    }
