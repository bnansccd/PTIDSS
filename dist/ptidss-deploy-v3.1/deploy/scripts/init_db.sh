...
echo "==> [2/3] 执行基线 DDL（01~16 幂等；TDengine 侧脚本 02 由时序库单独执行，见部署文档）"
# 10~16：平台配置/SPI 执行器/交易网关/模型任务/市场化区域/行情采集配置/血缘全量图谱（V2.2~V3.0 增量）
for f in 01_postgres_schema.sql 03_views.sql 06_enum_dict.sql 07_seed_data.sql \
         ...
         16_lineage_full_graph_v3_0.sql 17_sys_config_v3_0.sql; do
  if [ -f "$DDL_DIR/$f" ]; then
    echo "    -- 执行 $f"
